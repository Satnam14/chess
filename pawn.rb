    ...
    PAWN_MOVES.each do |candidate|
      ...
      if board.on_board?(possible_pos)
        if (pos[1] - possible_pos[1]).abs > 0
          @possible_moves << possible_pos if board.valid_attack?(pos, possible_pos) #&& self.moves_into_check?(possible_pos)
        else
          @possible_moves << possible_pos unless board.occupied?(possible_pos) #&& self.moves_into_check?(possible_pos)
        end
      end
    end
